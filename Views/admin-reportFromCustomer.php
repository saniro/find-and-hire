...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="Styles/admin-reportFromCustomerModalStyles.css">
	</head>
	<body>
		<?php
			if(!isset($_GET['sort'])){
						...
						<tr>
									...
									<ul class = "nothing">
	  									<li id = "dropdownSortID" class="dropdownSort">
	    									...
	    									<div class="dropdown-content">
	    										...
	    										?>
	    										<a href="index?route=reportFromCustomer&sort=reportsID<?php echo $optionSearch . $search; ?>">ID</a>
	      										<a href="index?route=reportFromCustomer&sort=customerName<?php echo $optionSearch . $search; ?>">Customer Name</a>
	      										<a href="index?route=reportFromCustomer&sort=reportType<?php echo $optionSearch . $search; ?>">Report Type</a>
	      										<a href="index?route=reportFromCustomer&sort=date<?php echo $optionSearch . $search; ?>">Date</a>
	    									</div>
	  									</li>
							...
							<th class = "searchCol" colspan="2">
								...
								<div class = "searchClass">
										<a href = "index?route=reportFromCustomer"><img class = "resetAction" src="Resources/reset.png"></a>
										...
										</select>&nbsp<input name = "search" class = "searchInput" type = "search" placeholder="Search here..." pattern="[^'\x22]+" title="Must not contain quotations."><input type = "hidden" name = "route" value = "reportFromCustomer">&nbsp<button name="searchButton" class = "searchBtn">SEARCH</button>
								</div>
							</form>
							</th>
						</tr>
  				...
  				<div class="viewModal-content">
    				...
    				<?php
    					if(isset($_GET['reportID'])){
    						...
    						(SELECT concat(lastName, ', ', firstName, ' ', lastName) FROM users WHERE userID = reporterID) AS customerName, 
    						(SELECT email FROM users WHERE userID = reporterID) AS customerEmail, 
    						(SELECT contact FROM users WHERE userID = reporterID) AS customerContactNo, 
		    				...
		    				$_SESSION["reportCustomerIDView"] = $reportCustomerIDView = $rowReportView["customerID"];
		...
		<div id="viewReviewModal" class="viewReviewModal">
			...
			<?php
				if(isset($_POST['sendNotif'])){
					...
					echo "<script type='text/javascript'>alert('Submitted successfully.');window.location.href='index?route=reportFromCustomer&sort=".$_SESSION['linkReportCustomer'].$optionSearch.$search."';</script>";
				}
			?>
		</div>
		</form>
		...
		<script type="text/javascript">
			...
			function viewReview(){
			    ...
			    span.onclick = function() {
					...
					var url = "index?route=reportFromCustomer"+linkSort+search+optionSearch
				...
				if(table!=null){
					for(var x = 1;x<table.rows.length;x++){
						table.rows[x].onclick = function(){
							...
							var search = '<?php if(isset($_GET['search'])){echo '&search=' . $_GET['search'];}?>'
							var optionSearch = '<?php if(isset($_GET['optionSearch'])){echo '&optionSearch=' . $_GET['optionSearch'];}?>'
							var url = "index?route=reportFromCustomer&reportID="+reportID+linkSort+search+optionSearch
							location.replace(url);
