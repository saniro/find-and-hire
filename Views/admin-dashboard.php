...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="Styles/admin-dashboardStyles.css">
	</head>
	<body>
		...
		<div>
			...
			<div class = wrapper>
				  ...
				  //   	$sqlLastSixMonthsBook = "SELECT DATE_FORMAT(date, '%M') AS date, count(bookingID) AS bookCount, (SELECT name FROM services AS SS WHERE SS.serviceID = BG.serviceID) AS serviceName FROM booking AS BG WHERE date > DATE_SUB(NOW(), INTERVAL 6 MONTH) GROUP BY DATE_FORMAT(date, '%M') ORDER BY date DESC";
						// $stmt = $con->prepare($sqlLastSixMonthsBook);
						// $stmt->execute();
						// $results = $stmt->fetchAll();
						// $rowCountBook = $stmt->rowCount();
						// $ctr = 0;
						// foreach($results as $rowLastSixMonthsBook){
						// 	$lastSixMonthChartTranMonth[$ctr] = $rowLastSixMonthsBook["date"];
						// 	$lastSixMonthChartBookCount[$ctr] = $rowLastSixMonthsBook["bookCount"];
						// 	$ctr++;
				  //   	}
